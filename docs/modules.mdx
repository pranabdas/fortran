...
<CodeBlock language="fortran" title="src/11_const_module.f90">{const_module}</CodeBlock>

Now we write a program that uses the above module:

import circ_area from '!!raw-loader!/src/11_circ_area.f90';

<CodeBlock language="fortran" title="src/11_circ_area.f90">{circ_area}</CodeBlock>
